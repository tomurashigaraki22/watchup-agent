...
main() {
    ...
    echo ""
    echo "⚠️  IMPORTANT: Register the agent before starting the service"
    echo ""
    echo "Next steps:"
    echo "1. Run the agent manually to register:"
    echo "   sudo /usr/local/bin/watchup-agent"
    echo ""
    echo "2. Enter your credentials when prompted:"
    echo "   - Project ID (from Watchup dashboard)"
    echo "   - Master API Key (from Watchup settings)"
    echo "   - Server Identifier (e.g., 'production-api')"
    echo ""
    echo "3. After successful registration, press Ctrl+C"
    echo ""
    echo "4. Start the service:"
    echo "   sudo systemctl start watchup-agent"
    echo ""
    echo "5. Check status:"
    echo "   sudo systemctl status watchup-agent"
    echo ""
    echo "6. View logs:"
    echo "   sudo journalctl -u watchup-agent -f"
    echo ""
